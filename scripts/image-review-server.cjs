...
<body>
  <div class="card">
    ...
    <p><strong>${groups.length}</strong> groupes — prochain à valider : <strong>${pending + 1}</strong></p>
...
<head>
  ...
  <style>
    ...
    .muted { color:#666; }
    .actions a, button { display:inline-block; margin:8px 8px 8px 0; padding:10px 14px; border-radius:10px; text-decoration:none; border:0; cursor:pointer; }
    a { background:#eee; color:#111; }
    button { background:#1f2937; color:white; }
    .validate { background:#047857; }
    .skip { background:#b45309; }
...
<body>
  <div class="card">
    <p class="muted">Groupe ${index + 1} / ${groups.length}</p>
